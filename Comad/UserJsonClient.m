...
- (void)getAddFriendInfo:(void (^)(AFHTTPRequestOperation *, NSHTTPURLResponse *, id))success failure:(void (^)(int, NSString *))failure {
    NSURL *url = [NSURL URLWithString:@"http://localhost:3000/api/user/get_add_friends?user_id=10"];
...
- (void)findUserWithComadId:(void (^)(AFHTTPRequestOperation *, NSHTTPURLResponse *, id))success failure:(void (^)(int, NSString *))failure {
    NSURL *url = [NSURL URLWithString:@"http://localhost:3000/api/user/find_user?user_id=10&comad_id=yuuki1224"];
    NSURLRequest *request = [NSURLRequest requestWithURL:url];
